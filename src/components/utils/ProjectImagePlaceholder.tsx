...
const variantClasses = {
  ...
  feature: {
    shell: 'p-6 sm:p-8 md:p-12',
    monogram: 'text-6xl sm:text-7xl md:text-8xl lg:text-9xl',
    title: 'text-xl sm:text-2xl md:text-3xl',
    footer: 'gap-2 sm:gap-4'
  }
} as const;
...
function ProjectImagePlaceholder({
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-3 top-3 h-14 w-14 border border-primary/20 dark:border-primary/45 sm:right-4 sm:top-4 sm:h-16 sm:w-16" />
      <div className={`absolute bottom-3 right-3 font-black tracking-[-0.08em] text-black/[0.05] dark:text-white/[0.1] sm:bottom-4 sm:right-4 ${styles.monogram}`}>
        {monogram}
      </div>
      ...
      >
        <div className="flex items-start justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-3">
            <div className="h-px w-8 bg-primary/70 shadow-[0_0_12px_rgba(37,99,235,0.35)] sm:w-10" />
            <span className="rounded-full border border-black/10 bg-white/70 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.3em] text-black/75 shadow-sm backdrop-blur-sm dark:border-primary-light/40 dark:bg-black/35 dark:text-primary-lighter dark:shadow-[0_0_0_1px_rgba(147,197,253,0.12)] sm:px-3 sm:text-[10px] sm:tracking-[0.34em]">
              {year}
            </span>
          </div>
          <div className="flex h-9 w-9 items-center justify-center border border-black/10 bg-white/55 text-black/55 backdrop-blur-sm dark:border-white/20 dark:bg-white/10 dark:text-white/70 sm:h-10 sm:w-10">
        ...
        <div className="space-y-4 sm:space-y-5">
          <div className="inline-flex max-w-full items-center border border-black/10 bg-white/65 px-3 py-1.5 backdrop-blur-sm dark:border-primary-light/60 dark:bg-primary/22 dark:shadow-[0_0_0_1px_rgba(96,165,250,0.18)]">
            <span className="truncate text-[9px] font-black uppercase tracking-[0.24em] text-primary dark:text-white sm:text-[10px] sm:tracking-[0.28em]">
              {category}
            </span>
          </div>

          <div className={`max-w-[18rem] font-black tracking-tighter leading-[0.9] text-black dark:text-white sm:max-w-[22rem] md:max-w-[26rem] ${styles.title}`}>
            {title}
          </div>

          <div className={`flex flex-wrap items-center text-[9px] font-black uppercase tracking-[0.24em] text-black/55 dark:text-white/65 sm:text-[10px] sm:tracking-[0.3em] ${styles.footer}`}>
            <span>{location}</span>
